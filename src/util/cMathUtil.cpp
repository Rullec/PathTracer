...
bool cMathUtil::IsPoint(const tVector &v) { return std::abs(v[3] - 1) < 1e-10; }

bool cMathUtil::IsVector(const tVector &v) { return std::abs(v[3]) < 1e-10; }
...
double cMathUtil::Radians(double degree) { return degree / 180.0 * M_PI; }
...
tQuaternion cMathUtil::RotFrom2Vec(const tVector &origin, const tVector &target)
{
    ...
    if ((origin + target).norm() < 1e-10)
    {
        int i = -1;
        for (i = 0; i < 3; i++)
            if (std::fabs(origin[i]) > 1e-6)
                break;
        axis = tVector::Random();
        axis[3] = 0;
        axis[i] = -(axis[(i + 1) % 3] * origin[(i + 1) % 3] +
                    axis[(i + 2) % 3] * origin[(i + 2) % 3]) /
                  origin[i];
        axis.normalize();
        angle = M_PI;
                           ...
                           const tVector &p3, double eps /* = 1e-5*/)
{
    ...
    // std::cout <<"ray cast = " << res.transpose() << std::endl;
    if (false ==
        (0 - eps < alpha && alpha < 1 + eps && 0 - eps < beta &&
         beta < 1 + eps && 0 - eps < gamma && gamma < 1 + eps && t > 0 - eps))
    {
        t = std::nan("");
    }
...
tVector cMathUtil::SampleHemiSphereUniform(const tVector &normal, double &pdf)
{
    ...
    tVector res = tVector(std::cos(theta) * std::cos(phi), std::sin(theta),
                          -std::cos(theta) * std::sin(phi), 0);
    pdf = 1.0 / (2 * M_PI);
    ...
    if (cMathUtil::IsNormalized(axis) == false)
    {
        ...
        {
            std::cout << "[error] cMathUtil::SampleHemiSphereUniform normal = "
...
*/
tVector cMathUtil::SampleCone(const tVector &medial_axis,
                              const double max_theta, double &pdf)
{
    ...
    double phi = 2 * M_PI * drand48(),
           theta = std::acos(1 - xi2 + xi2 * std::cos(max_theta));
