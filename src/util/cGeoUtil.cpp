...
tVector cGeoUtil::CalcPlaneEquation(const tVector &p1, const tVector &p2,
                                    const tVector &p3)
{
    ...
    {
        std::cout << "[error] cGeoUtil::CalculatePlaneEquation can not "
                     "construct a plane"
                  << std::endl;
    }
...
tVector cGeoUtil::SamplePointFromPlane(const tVector &equation)
{
    ...
    double x = std::rand(), y = std::rand(),
           z = (equation[3] + equation[0] * x + equation[1] * y) /
               (-equation[2]);
...
bool cGeoUtil::CalcLineLineIntersection(double x1, double y1, double x2,
                                        double y2, double y0, double &ixOut)
{
...
tVector cGeoUtil::CalcNormalFrom3Pts(const tVector &p1, const tVector &p2,
                                     const tVector &p3)
{
...
tVector cGeoUtil::Refract(const tVector &normal_, const tVector &incoming,
                          double ni)
{
    ...
    // assert(ni > 1.0);
    {
		...
		*/
        ...
        outgoing = ratio * (normal.cross3(-nxi)) -
                   normal * std::sqrt(1 - ratio * ratio * nxi.dot(nxi));
    }
...
tVector cGeoUtil::CalcBarycentricCoordinate(const tVector &p_, const tVector &a,
                                            const tVector &b, const tVector &c)
{
    ...
    tVector normal1 = cGeoUtil::CalcNormalFrom3Pts(p_, a, b),
            normal2 = cGeoUtil::CalcNormalFrom3Pts(p_, b, c);
    if (false == cMathUtil::IsSame(normal1, normal2, 1e-4))
    {
        ...
        std::cout << "normal1 = " << normal1.transpose()
                  << ", normal2 = " << normal2.transpose() << std::endl;
        // exit(1);
    }
