...
void cObjMesh::AddFace(tFace *f)
{
	...
	// set up shape info
	assert(f->mShapeId >= 0);
	while(mShapeFaceId.size() <= f->mShapeId) mShapeFaceId.push_back(std::vector<int>());
...
std::vector<std::pair<tVector, tAABB *>> cObjMesh::ShapeAnalysis(std::vector<std::vector<int>> & vec)
{
	...
	auto UpdateBound = [](tAABB & aabb, const tVector & pos)
	{
		for (int i = 0; i < 3; i++)
		{
			if(aabb.bound[i][0] > pos[i]) aabb.bound[i][0] = pos[i];
			if(aabb.bound[i][1]<pos[i]) aabb.bound[i][1] = pos[i];
		}
