...
public:
    tRay()
        : mOri(tVector::Zero()), mDir(tVector::Zero()){
    ...
    void Init(const tVector &ori, const tVector &dir)
    {
        ...
        for (int i = 0; i < 3; i++)
            if (std::isinf(mInvDir[i]))
                mInvDir[i] = 1e10 * (mDir[0] > 0 ? 1 : -1);
...
protected:
    ...
    tVector mUpperBound,
        mLowerBound; // upper & lower bound for bouding box of this mesh
