...
void BuildLinesForBox(std::vector<tLine> &lines, const tAABB &box)
{
    ...
    tVector low = tVector(box.bound[0][0], box.bound[1][0], box.bound[2][0], 1),
            high =
                tVector(box.bound[0][1], box.bound[1][1], box.bound[2][1], 1);
    tLine line;
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        for (int j = 0; j < 4; j++)
        {
            line.mOri[label1] = line.mDest[label1] =
                (j % 2 == 0) ? low[label1] : high[label1];
            line.mOri[label2] = line.mDest[label2] =
                (j / 2 == 0) ? low[label2] : high[label2];
            lines.push_back(line);
        }
    }
}
...
{
    tRay r1(line.mOri, line.mDest - line.mOri),
        r2(line.mDest, line.mOri - line.mDest);
    return intersect(r1) && intersect(r2);
}

void Project(std::vector<tVector> points, const tVector &axis, double &pt_min,
             double &pt_max)
{
...
bool tAABB::intersect(const tFace *face) const
{
    ...
    for (int i = 0; i < NUM_VERTEX_PER_FACE; i++)
        triangle_ver[i] = face->mVertexPtrList[i]->mPos;
    for (int x = 0; x < 2; x++)
        for (int y = 0; y < 2; y++)
            for (int z = 0; z < 2; z++)
            {
                aabb_ver[x * 4 + y * 2 + z] =
                    tVector(bound[0][x], bound[1][y], bound[2][z], 1);
    ...
    {
        tVector v1 = face->mVertexPtrList[1]->mPos -
                     face->mVertexPtrList[0]->mPos,
                v2 = face->mVertexPtrList[2]->mPos -
                     face->mVertexPtrList[1]->mPos;
...
//---------------------cBaseMesh------------------------
cBaseMesh::cBaseMesh(const eMeshType type, const std::string &filename)
    : mMeshType(type), mMeshPath(filename)
{
    mFaceNum = 0;
    mVertexNum = 0;
    mFaceList.clear();
...
void cBaseMesh::AddVertex(tVertex *v_)
{
    ...
    mVertexList.reserve(
        std::max(v_->mVertexId, static_cast<int>(mVertexList.size() + 1)));
    ...
    else
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            if (v_->mPos[i] > mUpperBound[i])
                mUpperBound[i] = v_->mPos[i];
            if (v_->mPos[i] < mLowerBound[i])
                mLowerBound[i] = v_->mPos[i];
        }
    }
}

std::vector<tVertex *> &cBaseMesh::GetVertexList() { return mVertexList; }

std::vector<tFace *> &cBaseMesh::GetFaceList() { return mFaceList; }

int cBaseMesh::GetVertexNum() { return mVertexNum; }

int cBaseMesh::GetFaceNum() { return mFaceNum; }

tVector cBaseMesh::GetCenter() { return mCenterPos; }

eMeshType cBaseMesh::GetType() { return mMeshType; }

void cBaseMesh::PrintInfo()
{
    printf("[log] obj mesh includes %d faces and %d vertices", mFaceNum,
           mVertexNum);
}
