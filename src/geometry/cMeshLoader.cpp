...
std::shared_ptr<cBaseMesh> cMeshLoader::Load(const tMeshParams &params_)
{
    ...
    case OBJ:
    {
        ...
        std::shared_ptr<cObjMesh> x =
            LoadObj(params_.name, params_.scale, params_.displacement,
                    params_.build_edge_info, params_.shape_analysis);
        mesh_ = std::dynamic_pointer_cast<cBaseMesh>(x);
    }
    break;
    default:
    {
        std::cout << "[error] cMeshLoader::Load: Unsupported mesh type "
                  << params_.type << std::endl;
        exit(1);
    }
    break;
    }
...
std::shared_ptr<cObjMesh>
cMeshLoader::LoadObj(const std::string &name, double scale, const tVector &disp,
                     bool build_edge, bool shape_analysis)
{
    ...
    if (false == tinyobj::LoadObj(
                     &attrib, &shapes, &materials, nullptr, nullptr,
                     name.c_str(),
                     cFileUtil::GetDirname(name).c_str())) //, "./model/cbox"
    {
        std::cout << "[error] cMeshLoader::LoadObj " << name << " failed "
                  << std::endl;
        exit(1);
    }
    ...
    for (auto &x : materials)
    {
        ...
        double ior = x.ior, shininess = x.shininess, illum = x.illum;
    ...
    for (size_t shape_id = 0; shape_id < shapes.size(); shape_id++)
    {
        ...
        for (size_t face_id = 0;
             face_id < shapes[shape_id].mesh.num_face_vertices.size();
             face_id++)
        {
            // if(face_id % 1000 == 0)
            ...
            {
                std::cout << "[error] cMeshLoader::LoadObj: didn't support "
                             "faces with "
                          << fv << " vertices" << std::endl;
                exit(1);
            }

            tFace *cur_face = new tFace();
            cur_face->mFaceId = face_id_before + face_id;
            cur_face->mMaterialId =
                shapes[shape_id]
                    .mesh
                    .material_ids[face_id]; // make the material id count from 0
            ...
            for (size_t v = 0; v < fv; v++)
            {
                ...
                tinyobj::index_t idx =
                    shapes[shape_id].mesh.indices[index_offset + v];
                //std::cout << "vertex = " << idx.vertex_index << std::endl;
                tinyobj::real_t vx =
                    (attrib.vertices[3 * idx.vertex_index + 0] + disp[0]) *
                    scale;
                tinyobj::real_t vy =
                    (attrib.vertices[3 * idx.vertex_index + 1] + disp[1]) *
                    scale;
                tinyobj::real_t vz =
                    (attrib.vertices[3 * idx.vertex_index + 2] + disp[2]) *
                    scale;

                // load vertices normal
                tinyobj::real_t nx = std::nan(""), ny = std::nan(""),
                                nz = std::nan("");
