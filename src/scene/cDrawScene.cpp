...
cDrawScene::cDrawScene(const std::string &config) : cScene()
{
    ...
    if (mRender == nullptr)
    {
        std::cout << "[error] cDrawScene set up pixel render failed "
                  << std::endl;
        exit(1);
    }
...
void cDrawScene::Init()
{
    ...
    {
        std::cout << "[error] cDrawScene::Init status error: "
                  << cScene::mSceneStatusName[mSceneStatus] << std::endl;
        exit(1);
    }
...
void cDrawScene::DrawAxis()
{
    ...
    tEdge cur;
    cur.mOri = new tVertex();
    cur.mDest = new tVertex();
    for (int i = 0; i < 3; i++)
    {
        tVector ori_pos = tVector::Zero(), dest_pos = tVector::Zero(),
                color = tVector::Zero();
        ori_pos[3] = 1;
        dest_pos[3] = 1;
        dest_pos[i] = 1000;
...
void cDrawScene::ScrollEvent(double offset) { mCamera->ScrollEvent(offset); }
