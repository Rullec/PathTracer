...
const std::string gStrSceneType[eSceneType::NUM_SCENE_TYPE] = {
	"DrawScene",
	"RTScene"
};
...
std::shared_ptr<cScene> BuildScene(std::string path)
{
	...
	{
		std::cout <<"[error] BuildScene: parse json error: " << path << std::endl;
		exit(1);
	}
	...
	if(scene == nullptr)
	{
		std::cout <<"[error] BuildScene: Check scene type failed: " << scene_type<<std::endl;
		exit(1);
	}
