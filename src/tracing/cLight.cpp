...
eLightType cLight::GetType() { return mType; }

cSquareLight::cSquareLight(const tVector light_pt[4], const tVector &v)
    : cLight(eLightType::SQUARE, v)
{
    // std::cout <<"[debug] init square light begin\n";
    tVector normal1 = cGeoUtil::CalcNormalFrom3Pts(light_pt[0], light_pt[1],
                                                   light_pt[2]),
            normal2 = cGeoUtil::CalcNormalFrom3Pts(light_pt[1], light_pt[2],
                                                   light_pt[3]);
    ...
    mArea = (mLightPos[2] - mLightPos[1]).norm() *
            (mLightPos[1] - mLightPos[0]).norm();
...
void cSquareLight::Sample_Li(const tVector &ref, tRay &wi_ray, double *pdf)
{
    ...
    else
    {
        ...
        *pdf = pow((sample_pt - ref).norm(), 2) /
               (mArea * ray_dir.dot(mNormal)) * (1.0 / (2 * M_PI));
...
cSphereLight::cSphereLight(const tVector &center, double radius,
                           const tVector &radiance)
    : mCenter(center), mRadius(radius), cLight(eLightType::SPHERE, radiance)
{
    ...
    // init face
    {
        for (int l = 0; l <= layers; l++)
            for (int s = 0; s <= samples; s++)
            {
                ...
                cur_v.mPos.x() =
                    std::cos(theta) * local_radius * radius + center[0];
                cur_v.mPos.y() =
                    std::sin(theta) * local_radius * radius + center[1];
        ...
        for (int l = 0; l < layers; l++)
            for (int s = 0; s < samples; s++)
            {
                tVertex &v1 = vertices[l][s], &v2 = vertices[l + 1][s],
                        &v3 = vertices[l + 1][s + 1], &v4 = vertices[l][s + 1];
...
void cSphereLight::Sample_Li(const tVector &ref, tRay &wi_ray, double *pdf)
{
    ...
    if (dc > mRadius)
    {
        ...
        tVector sample_dir =
            cMathUtil::SampleCone(from_ref_to_light, max_theta, *pdf);
        ...
        double ds =
            dc * std::cos(theta) -
            std::sqrt(mRadius * mRadius - dc * dc * pow(std::sin(theta), 2));
    ...
    else
    {
        ...
        tVector light_pt =
            cMathUtil::SampleSphereUniform(*pdf) * mRadius + mCenter;
...
std::vector<std::shared_ptr<cLight>> BuildLight(const std::string &conf)
{
    ...
    if (scale < 1e-5 || light_num == 0 || light_type_lst.isNull() ||
        light_info_lst.isNull() || light_radiance_lst.isNull())
    {
        std::cout << "[error] BuildLight light parse failed " << scale
                  << light_num << light_info_lst.isNull()
                  << light_type_lst.isNull() << light_radiance_lst.isNull()
                  << std::endl;
        exit(1);
    }
    ...
    for (int light_id = 0; light_id < light_num; light_id++)
    {
        ...
        // std::cout <<"----------------\n";
        for (int i = 0; i < 3; i++)
            light_radiance[i] = light_radiance_lst[light_id][i].asDouble();
        // std::cout <<"radiance = " << light_radiance.transpose() << std::endl;
        if (type == "square")
        {
            tVector square_pts[4] = {tVector::Zero(), tVector::Zero(),
                                     tVector::Zero(), tVector::Zero()};
            Json::Value sub_pt;
            for (int i = 0; i < 4; i++)
            {
                sub_pt = light_info[i];
            ...
            light_res_lst[light_id] = std::shared_ptr<cLight>(
                new cSquareLight(square_pts, light_radiance));
        ...
        else if (type == "sphere")
        {
            ...
            double radius;
            for (int i = 0; i < 3; i++)
                sphere_center[i] = light_info[i].asDouble() * scale;
            radius = light_info[3].asDouble() * scale;
            light_res_lst[light_id] = std::shared_ptr<cLight>(
                new cSphereLight(sphere_center, radius, light_radiance));
        }
        else
        {
            std::cout << "[error] BuildLight: Unsupported type " << type
                      << std::endl;
            exit(1);
        }
    }
