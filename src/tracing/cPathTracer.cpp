...
void cPathTracer::Init(std::shared_ptr<cBaseMesh> scene_mesh,
...
void cPathTracer::InitBxDF()
{
    std::shared_ptr<cObjMesh> obj_mesh =
        std::dynamic_pointer_cast<cObjMesh>(mSceneMesh);
    if (obj_mesh == nullptr)
    {
        std::cout << "[error] cPathTracer::BuildBxDF failed, empty mesh!"
                  << std::endl;
        exit(1);
    }
...
void cPathTracer::GenerateRay()
{
    ...
    */
    ...
    tMatrix mat;
    {
        ...
        mat3(0, 0) = mWidth * 1.0 / mHeight *
                     std::tan(cMathUtil::Radians(mFov) / 2) * mNear;
...
void cPathTracer::ParseConfig(const std::string &conf)
{
    ...
    mEnableBarycentricNormal =
        path_tracer_json["EnableBarycentricNormal"].asBool();
    ...
    // exit(1);
    {
        ...
        assert(DrawRegion_json.isNull() == false &&
               DrawRegion_json.size() == 4);
        ...
        printf("[debug] draw region = (%d, %d) to (%d, %d)\n", mDrawRegion.stX,
               mDrawRegion.stY, mDrawRegion.edX, mDrawRegion.edY);
    }
...
void cPathTracer::GetDrawResources(std::vector<tLine> &lines,
...
#pragma omp parallel for schedule(dynamic)
    ...
    for (int x = 0; x < mHeight; x++)
    {
        ...
        if (x < mDrawRegion.stX || x > mDrawRegion.edX)
            continue;
        for (int y = 0; y < mWidth; y++)
        {
            if (y < mDrawRegion.stY || y > mDrawRegion.edY)
                continue;
            // if(i%10 !=0) continue;
            int i = x * mWidth + y;
    ...
    for (int s = 0; s < this->mSamples; s++)
    {
        ...
        for (int bounce = 0; bounce < mMaxDepth; bounce++)
        {
            ...
            if (target_face == nullptr ||
                (mat_id = target_face->mMaterialId) == -1)
            {
                final_color = tVector::Zero();
                break;
            }
            ...
            {
                tVector bary_coords = cGeoUtil::CalcBarycentricCoordinate(
                    ref_pt, vertex_lst[0]->mPos, vertex_lst[1]->mPos,
                    vertex_lst[2]->mPos);
                ref_normal = tVector::Zero();
                for (int i = 0; i < 3; i++)
                    ref_normal += bary_coords[i] * vertex_lst[i]->mNormal;
                ref_normal.normalize();
            }
            else
            {
                ref_normal = cGeoUtil::CalcNormalFrom3Pts(vertex_lst[0]->mPos,
                                                          vertex_lst[1]->mPos,
                                                          vertex_lst[2]->mPos);
            }
            ...
            direct_color = bxdf->Sample_Li(mLight, mAccelStruct.get(),
                                           ref_normal, ref_pt, ray);
            ...
            if (mEnableIndrectLight == true)
            {
                ...
                tVector
                    wi_dir_oppo; // the definition of wi_dir is a incoming light ref_pt -> outside
                indirect_coeff =
                    bxdf->Sample_f(ref_normal, -ray.GetDir(), wi_dir_oppo);
        ...
        for (int i = direct_light_lst.size() - 1; i >= 0; i--)
        {
            final_color =
                direct_light_lst[i] +
                cMathUtil::WiseProduct(indirect_light_lst[i], final_color);
